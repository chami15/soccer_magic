"""
Teste manual escalonado v7: coleta os 5 jogos mais recentes da Austrália
e do Paraguai, persistindo tudo end-to-end.

Estrutura idêntica à v4/v5/v6 — mesma função processar_time(), mesmos dados
globais carregados uma única vez. Os dois times são localizados por nome
na lista de times da Copa, sem IDs hardcoded.

Rodar localmente:
    cd backend
    python3 test_pipeline_basicov7.py
"""
...
if __name__ == "__main__":
    with httpx.Client() as client:
        ...
        # Localizar Austrália e Paraguai na lista de times da Copa
        australia = next(
            t for t in teams
            if any(kw in t.get("name", "") for kw in ("Australia", "Austrália"))
        )
        paraguai = next(
            t for t in teams
            if any(kw in t.get("name", "") for kw in ("Paraguay", "Paraguai"))
        )
        print(f"Austrália encontrada: id={australia['id']} nome={australia['name']}")
        print(f"Paraguai encontrado: id={paraguai['id']} nome={paraguai['name']}")
        ...
        # ── Pipeline completo: Austrália ────────────────────────────────────────
        print(f"\n{'*' * 60}")
        print("*** PROCESSANDO: AUSTRÁLIA ***")
        print(f"{'*' * 60}")
        processar_time(
            client=client,
            team=australia,
        ...
        # ── Pipeline completo: Paraguai ──────────────────────────────────────────
        print(f"\n{'*' * 60}")
        print("*** PROCESSANDO: PARAGUAI ***")
        print(f"{'*' * 60}")
        processar_time(
            client=client,
            team=paraguai,
