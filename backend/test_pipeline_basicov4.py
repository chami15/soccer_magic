"""
Teste manual escalonado v4: coleta os 5 jogos mais recentes do Brasil
e os 5 jogos mais recentes da Escócia, persistindo tudo end-to-end.

Diferença da v3: processa dois times (Brasil e Escócia) em vez de um.
A Escócia é buscada por nome na lista de times da Copa para não depender
de um ID hardcoded.

Estrutura geral:
  [uma vez, globais]
  - season_id + lista de times da Copa (para localizar ambos os times)
  - lista de rounds + metadados do round de power ranking
  - todos os rankings do round (filtrado por time dentro da função)

  [para cada time: Brasil e Escócia]
  1. dim_selecao (time principal)
  2. fato_power_ranking_selecao
  3. performance_points do time
  4. buscar últimos 5 jogos
  [para cada uma das 5 partidas]
  5. dim_selecao adversário
  6. fato_partida
  7. fato_estatistica_selecao_partida (time principal + adversário)
  8. dim_jogador + fato_evento_partida
  9. fato_h2h_evento

Rodar localmente:
    cd backend
    python3 test_pipeline_basicov4.py
"""
...
def processar_time(
    ...
    team      — dict no formato de get_wc_teams (campos: id, name, country, group_name, ranking_fifa)
    rankings  — lista completa do round já carregada (evita chamada duplicada à API)
    label     — texto de identificação para os prints (ex: 'Brasil', 'Escócia')
    """
    team_id = team["id"]
...
if __name__ == "__main__":
    with httpx.Client() as client:
        ...
        # Localizar Brasil e Escócia na lista de times da Copa
        brasil = next(t for t in teams if t["id"] == BRASIL_ID)
        escocia = next(
            t for t in teams
            if "Scotland" in t.get("name", "") or "Escócia" in t.get("name", "")
        )
        ESCOCIA_ID = escocia["id"]
        print(f"Brasil encontrado: id={brasil['id']} nome={brasil['name']}")
        print(f"Escócia encontrada: id={escocia['id']} nome={escocia['name']}")
        ...
        # ── Pipeline completo: Escócia ────────────────────────────────────────
        print(f"\n{'*' * 60}")
        print("*** PROCESSANDO: ESCÓCIA ***")
        print(f"{'*' * 60}")
        processar_time(
            client=client,
            team=escocia,
