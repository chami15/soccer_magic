"""
Teste manual escalonado v8: coleta os 5 jogos mais recentes do Uruguai
e da Espanha, persistindo tudo end-to-end.

Estrutura idêntica à v4/v5/v6/v7 — mesma função processar_time(), mesmos dados
globais carregados uma única vez. Os dois times são localizados por nome
na lista de times da Copa, sem IDs hardcoded.

Rodar localmente:
    cd backend
    python3 test_pipeline_basicov8.py
"""
...
if __name__ == "__main__":
    with httpx.Client() as client:
        ...
        # Localizar Uruguai e Espanha na lista de times da Copa
        uruguai = next(
            t for t in teams
            if any(kw in t.get("name", "") for kw in ("Uruguay", "Uruguai"))
        )
        espanha = next(
            t for t in teams
            if any(kw in t.get("name", "") for kw in ("Spain", "Espanha", "España"))
        )
        print(f"Uruguai encontrado: id={uruguai['id']} nome={uruguai['name']}")
        print(f"Espanha encontrada: id={espanha['id']} nome={espanha['name']}")
        ...
        # ── Pipeline completo: Uruguai ───────────────────────────────────────────
        print(f"\n{'*' * 60}")
        print("*** PROCESSANDO: URUGUAI ***")
        print(f"{'*' * 60}")
        processar_time(
            client=client,
            team=uruguai,
        ...
        # ── Pipeline completo: Espanha ───────────────────────────────────────────
        print(f"\n{'*' * 60}")
        print("*** PROCESSANDO: ESPANHA ***")
        print(f"{'*' * 60}")
        processar_time(
            client=client,
            team=espanha,
