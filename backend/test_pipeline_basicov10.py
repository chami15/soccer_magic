"""
Teste manual escalonado v10: coleta os 5 jogos mais recentes da Nova Zelândia
e da Bélgica, persistindo tudo end-to-end.

Estrutura idêntica à v4/v5/v6/v7/v8/v9 — mesma função processar_time(), mesmos
dados globais carregados uma única vez. Os dois times são localizados por
nome na lista de times da Copa, sem IDs hardcoded.

Rodar localmente:
    cd backend
    python3 test_pipeline_basicov10.py
"""
...
if __name__ == "__main__":
    with httpx.Client() as client:
        ...
        # Localizar Nova Zelândia e Bélgica na lista de times da Copa
        nova_zelandia = next(
            t for t in teams
            if any(kw in t.get("name", "") for kw in ("New Zealand", "Nova Zelândia", "Nova Zelandia"))
        )
        belgica = next(
            t for t in teams
            if any(kw in t.get("name", "") for kw in ("Belgium", "Bélgica", "Belgica"))
        )
        print(f"Nova Zelândia encontrada: id={nova_zelandia['id']} nome={nova_zelandia['name']}")
        print(f"Bélgica encontrada: id={belgica['id']} nome={belgica['name']}")
        ...
        # ── Pipeline completo: Nova Zelândia ─────────────────────────────────────
        print(f"\n{'*' * 60}")
        print("*** PROCESSANDO: NOVA ZELÂNDIA ***")
        print(f"{'*' * 60}")
        processar_time(
            client=client,
            team=nova_zelandia,
        ...
        # ── Pipeline completo: Bélgica ────────────────────────────────────────────
        print(f"\n{'*' * 60}")
        print("*** PROCESSANDO: BÉLGICA ***")
        print(f"{'*' * 60}")
        processar_time(
            client=client,
            team=belgica,
