"""
Teste manual escalonado v5: coleta os 5 jogos mais recentes da Costa do Marfim
e de Curaçao, persistindo tudo end-to-end.

Estrutura idêntica à v4 — mesma função processar_time(), mesmos dados globais
carregados uma única vez. Os dois times são localizados por nome na lista de
times da Copa, sem IDs hardcoded.

Rodar localmente:
    cd backend
    python3 test_pipeline_basicov5.py
"""
...
if __name__ == "__main__":
    with httpx.Client() as client:
        ...
        # Localizar Costa do Marfim e Curaçao na lista de times da Copa
        costa_marfim = next(
            t for t in teams
            if any(kw in t.get("name", "") for kw in ("Ivory Coast", "Côte d'Ivoire", "Cote d'Ivoire", "Costa do Marfim"))
        )
        curacao = next(
            t for t in teams
            if any(kw in t.get("name", "") for kw in ("Curaçao", "Curacao"))
        )
        print(f"Costa do Marfim encontrada: id={costa_marfim['id']} nome={costa_marfim['name']}")
        print(f"Curaçao encontrado: id={curacao['id']} nome={curacao['name']}")
        ...
        # ── Pipeline completo: Costa do Marfim ────────────────────────────────
        print(f"\n{'*' * 60}")
        print("*** PROCESSANDO: COSTA DO MARFIM ***")
        print(f"{'*' * 60}")
        processar_time(
            client=client,
            team=costa_marfim,
        ...
        # ── Pipeline completo: Curaçao ────────────────────────────────────────
        print(f"\n{'*' * 60}")
        print("*** PROCESSANDO: CURAÇAO ***")
        print(f"{'*' * 60}")
        processar_time(
            client=client,
            team=curacao,
