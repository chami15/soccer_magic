"""
Teste manual escalonado v6: coleta os 5 jogos mais recentes do Japão
e da Suécia, persistindo tudo end-to-end.

Estrutura idêntica à v4/v5 — mesma função processar_time(), mesmos dados
globais carregados uma única vez. Os dois times são localizados por nome
na lista de times da Copa, sem IDs hardcoded.

Rodar localmente:
    cd backend
    python3 test_pipeline_basicov6.py
"""
...
if __name__ == "__main__":
    with httpx.Client() as client:
        ...
        # Localizar Japão e Suécia na lista de times da Copa
        japao = next(
            t for t in teams
            if any(kw in t.get("name", "") for kw in ("Japan", "Japão", "Japon"))
        )
        suecia = next(
            t for t in teams
            if any(kw in t.get("name", "") for kw in ("Sweden", "Suécia", "Suecia"))
        )
        print(f"Japão encontrado: id={japao['id']} nome={japao['name']}")
        print(f"Suécia encontrada: id={suecia['id']} nome={suecia['name']}")
        ...
        # ── Pipeline completo: Japão ───────────────────────────────────────────
        print(f"\n{'*' * 60}")
        print("*** PROCESSANDO: JAPÃO ***")
        print(f"{'*' * 60}")
        processar_time(
            client=client,
            team=japao,
        ...
        # ── Pipeline completo: Suécia ─────────────────────────────────────────
        print(f"\n{'*' * 60}")
        print("*** PROCESSANDO: SUÉCIA ***")
        print(f"{'*' * 60}")
        processar_time(
            client=client,
            team=suecia,
